...
SELECT SUBSTRING(code, 1, 1) AS SUBSTRING, name
FROM countries;
...
SELECT countries.name AS '国名', cities.name AS '市区町村名', countrylanguages.language
FROM countries
JOIN cities
...
SELECT celebrities.name, countries.name, countrylanguages.language
FROM celebrities, countries, countrylanguages
WHERE celebrities.country_code = countries.code
AND (countries.code = countrylanguages.country_code AND countrylanguages.is_official = 'T');
...
UNION
SELECT '1981' AS '誕生年', COUNT(birth) AS '人数'
FROM celebrities
WHERE birth >= '1981-01-01'
